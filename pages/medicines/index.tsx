...
export default function Home() {
  ...
  useEffect(() => {
    // Fetch medicines data from medicines.json
  ...
  return (
    <div>
      ...
      <main className="container mx-auto my-8">
        <h1 className="text-2xl font-bold mb-4">Welcome to the Medicine Store</h1>

        <h2 className="text-xl font-semibold mb-4">Available Medicines:</h2>
        
        {/* Grid to display available medicines */}
          ...
          {medicines.map((medicine) => (
            <div key={medicine.id} className="border rounded-lg shadow-md overflow-hidden transition-transform transform hover:scale-105">
              ...
              <div className="p-4">
                ...
                {/* Link to medicine details page */}
                <Link href={`/medicines/${medicine.id}`}>
                  <a className="mt-2 inline-block bg-blue-500 text-white text-center py-2 px-4 rounded hover:bg-blue-600">
                    View Details
                  </a>
                </Link>
              </div>
